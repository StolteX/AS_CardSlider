...
Sub Class_Globals
	...
	Private g_x,g_LastOffset As Float 'Ignore
...
#End If
	Select Action
		...
		Case 1
			...
			isfirstmove = False
			m_LastIndex = m_Index
			If g_x < x Or g_LastOffset > xclv_Main.sv.ScrollViewOffsetX Then
				m_Index = Max(1,m_Index -1)
			Else If g_x > x Or g_LastOffset < xclv_Main.sv.ScrollViewOffsetX  Then
			...
			Else
				...
				#Else
				xclv_Main.ScrollToItem(item.Offset - (m_SideGap-0dip))
				#End If
				Return True
...
Private Sub xclv_ScrollChanged (Offset As Int)
	g_LastOffset = Offset
End Sub
